...
class CommissionPercentageByWeekday(models.Model):
    ...
    weekday = models.PositiveIntegerField(
        choices=WEEKDAYS, unique=True, null=False, blank=False
    )
...
class Sale(models.Model):
    ...
    customer = models.ForeignKey(
        Customer, on_delete=models.RESTRICT, null=False, blank=False
    )
    ...
    def calculate_sale_total_value(self):
        total_value = sum(
            product.calculate_total_price() for product in self.saleproduct_set.all()
        )
        return total_value

    def calculate_total_commission(self):
        total_commission = sum(
            product.calculate_product_commission()
            for product in self.saleproduct_set.all()
        )
        return total_commission

    def count_items(self):
        quantity_of_items = sum(
            product.quantity for product in self.saleproduct_set.all()
        )
